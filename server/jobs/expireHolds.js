...
function expireHolds(db) {
  ...
  const strikesBeforeCooldown = getSetting(db, 'strikes_before_cooldown') || 3;
  const cooldownHours = getSetting(db, 'cooldown_hours') || 48;
  const noShowFine = getSetting(db, 'no_show_fine') || 0;

  const expireSeat = db.transaction((seat) => {
    const seatUpdate = db.prepare(`UPDATE seats SET status = 'available',
        held_by = NULL, held_at = NULL, hold_expires_at = NULL WHERE id = ?`);
    const penaltyInsert = db.prepare(`INSERT INTO penalties (student_id, reason) VALUES (?, 'no_show')`);
    const studentUpdate = db.prepare(`UPDATE students
        SET strikes = strikes + 1, fine_balance = fine_balance + ?,
          cooldown_until = CASE WHEN strikes + 1 >= ? THEN datetime('now', '+' || ? || ' hours')
          ELSE cooldown_until END WHERE id = ?`);
    const entryLogUpdate = db.prepare(`UPDATE entry_logs SET status = 'no_show'
...
function startExpireHoldsJob(db, intervalMs = 30000) {
  // Run once immediately in case any holds expired while the server was down.
  expireHolds(db);

  return setInterval(() => {
    try { expireHolds(db); }
    catch (error) { console.error('Seat expiry job failed:', error); }
  }, intervalMs);
}
